...
def convert_coco_json(json_file, out_dir, use_segments=False, cls91to80=False):
    #save_dir = make_dirs()  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    #for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
    fn = Path(out_dir) #Path(save_dir) / 'labels' / json_file.stem.replace('instances_', '')  # folder name
    #fn.mkdir()
    with open(json_file) as f:
        data = json.load(f)
    # Create image dict
...
def coco_to_yolo(json_file,out_dir):
    source = 'COCO'

    if source == 'COCO':
        convert_coco_json(json_file, out_dir,
                          use_segments=False,
                          cls91to80=True)
